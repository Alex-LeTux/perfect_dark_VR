        ...
        if (count > 0) {
            if (roomnum == light->roomnum) {
                ...
                if (numgood > 0) {
                    ...
                    // 1. Calculate the physical 3D size (without using the 2D screen resolution)
                    f32 phys_w = stageGetCurrent()->light_width * brightness * 0.01f;
                    f32 phys_h = stageGetCurrent()->light_height * brightness * 0.01f;

                    static f32 vr_glare_scale = 1.0f;
                    mainOverrideVariable("glare", &vr_glare_scale);
                    phys_w *= vr_glare_scale;
                    phys_h *= vr_glare_scale;
                    ...
                    if (brightness > 3.0f) {
                        ...
                        struct textureconfig *texconfig = &g_TexLightGlareConfigs[stageGetCurrent()->light_type];
                        ...
                        texSelect(&gdl, texconfig, 4, 1, 2, 1, NULL);
                        ...
                        mtx00015be0(camGetWorldToScreenMtxf(), &viewRot);
                        viewRot.m[3][0] = 0.0f; viewRot.m[3][1] = 0.0f; viewRot.m[3][2] = 0.0f;

                        Mtxf *mtxL = gfxAllocateMatrix();
                        mtxF2L(&viewRot, mtxL);
                        ...
                        f32 cx = lightworldpos.f[0] - g_Vars.currentplayer->cam_pos.f[0];
                        f32 cy = lightworldpos.f[1] - g_Vars.currentplayer->cam_pos.f[1];
                        f32 cz = lightworldpos.f[2] - g_Vars.currentplayer->cam_pos.f[2];

                        f32 dist = sqrtf(cx*cx + cy*cy + cz*cz);
                        if (dist < 1.0f) dist = 1.0f;

                        f32 final_w = phys_w * dist * 0.0025f;
                        f32 final_h = phys_h * dist * 0.0025f;

                        Vtx *vertices = gfxAllocateVertices(4);
                        vertices[0].colour = vertices[1].colour = vertices[2].colour = vertices[3].colour = 0;

                        vertices[0].s = -maxS; vertices[0].t = -maxT;
                        vertices[1].s =  maxS; vertices[1].t = -maxT;
                        vertices[2].s =  maxS; vertices[2].t =  maxT;
                        vertices[3].s = -maxS; vertices[3].t =  maxT;

                        vertices[0].x = (s16)roundf(cx - right.x * final_w + up.x * final_h);
                        vertices[0].y = (s16)roundf(cy - right.y * final_w + up.y * final_h);
                        vertices[0].z = (s16)roundf(cz - right.z * final_w + up.z * final_h);

                        vertices[1].x = (s16)roundf(cx + right.x * final_w + up.x * final_h);
                        vertices[1].y = (s16)roundf(cy + right.y * final_w + up.y * final_h);
                        vertices[1].z = (s16)roundf(cz + right.z * final_w + up.z * final_h);

                        vertices[2].x = (s16)roundf(cx + right.x * final_w - up.x * final_h);
                        vertices[2].y = (s16)roundf(cy + right.y * final_w - up.y * final_h);
                        vertices[2].z = (s16)roundf(cz + right.z * final_w - up.z * final_h);

                        vertices[3].x = (s16)roundf(cx - right.x * final_w - up.x * final_h);
                        vertices[3].y = (s16)roundf(cy - right.y * final_w - up.y * final_h);
                        vertices[3].z = (s16)roundf(cz - right.z * final_w - up.z * final_h);
                        ...
                        if (extra) {
                            ...
                            colour[3] = (u8)(stageGetCurrent()->light_alpha * s3[0] / 8.0f);
                            gDPSetEnvColor(gdl++, colour[0], colour[1], colour[2], colour[3]);

                            f32 w_ext = final_w * 0.4f;
                            f32 h_ext = final_h * 0.4f;
                            ...
                            vertices2[0].s = -maxS; vertices2[0].t = -maxT;
                            vertices2[1].s =  maxS; vertices2[1].t = -maxT;
                            vertices2[2].s =  maxS; vertices2[2].t =  maxT;
                            vertices2[3].s = -maxS; vertices2[3].t =  maxT;

                            vertices2[0].x = (s16)roundf(ecx - right.x * w_ext + up.x * h_ext);
                            vertices2[0].y = (s16)roundf(ecy - right.y * w_ext + up.y * h_ext);
                            vertices2[0].z = (s16)roundf(ecz - right.z * w_ext + up.z * h_ext);

                            vertices2[1].x = (s16)roundf(ecx + right.x * w_ext + up.x * h_ext);
                            vertices2[1].y = (s16)roundf(ecy + right.y * w_ext + up.y * h_ext);
                            vertices2[1].z = (s16)roundf(ecz + right.z * w_ext + up.z * h_ext);

                            vertices2[2].x = (s16)roundf(ecx + right.x * w_ext - up.x * h_ext);
                            vertices2[2].y = (s16)roundf(ecy + right.y * w_ext - up.y * h_ext);
                            vertices2[2].z = (s16)roundf(ecz + right.z * w_ext - up.z * h_ext);

                            vertices2[3].x = (s16)roundf(ecx - right.x * w_ext - up.x * h_ext);
                            vertices2[3].y = (s16)roundf(ecy - right.y * w_ext - up.y * h_ext);
                            vertices2[3].z = (s16)roundf(ecz - right.z * w_ext - up.z * h_ext);
